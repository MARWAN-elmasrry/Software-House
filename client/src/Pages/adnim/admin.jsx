...
const DashboardIcon      = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>);
const ProjectsIcon       = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>);
const BlogIcon           = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>);
const RevenueIcon        = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>;
const ProjectsMetricIcon = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>;
const LeadsIcon          = () => <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>;
const Spinner            = () => <span style={{ display:"inline-block", width:14, height:14, border:"2px solid currentColor", borderTopColor:"transparent", borderRadius:"50%", animation:"spin 0.7s linear infinite" }} />;
...
// superadmin → all nav; web_ai → Projects + Blog only (no Dashboard)
const BASE_NAV = [
  ...
  const handleLogin = useCallback(async () => {
    ...
    try {
      ...
      if (newAttempts >= MAX_ATTEMPTS) {
        ...
        setMessage(`${err} — ${remaining} attempt${remaining !== 1 ? "s" : ""} remaining.`);
      }
    }
    ...
    <>
      ...
      <div className="login-shell">
        ...
        <div className="login-card">
          <div className="login-brand"><GridIcon /><span className="login-brand-name">Software Admin</span></div>
          ...
          <div className="login-field">
            ...
            <input type="text" value={username}
              ...
              onKeyDown={handleKeyDown} placeholder="username" autoComplete="off"
              disabled={isDisabled} className={`login-input${errors.username ? " error" : ""}`} />
          ...
          <div className="login-field">
            ...
            <input type="password" value={password}
              ...
              disabled={isDisabled} className={`login-input${errors.password ? " error" : ""}`} />
...
const Dashboard = () => {
  ...
  useEffect(() => {
    getStats().then((res) => setStats(res.data)).catch(setError).finally(() => setStatsLoading(false));
  }, []);

  useEffect(() => {
    GetContact().then(setTableRows).catch(setError).finally(() => setContactLoading(false));
  }, []);
  ...
  const metrics = statsLoading || !stats
    ...
    : [
        { label: "Total Revenue",   value: fmtCurrency(stats.totalRevenue), badge: `${stats.total} total`, Icon: RevenueIcon },
        { label: "Active Projects", value: stats.active ?? "—",             badge: "active",               Icon: ProjectsMetricIcon },
        { label: "Pending Leads",   value: tableRows.length,                badge: "contacts",             Icon: LeadsIcon },
      ];
  ...
  return (
    <>
      ...
      {error && <div style={{ color:"#fc8181", marginBottom:"1rem" }}>⚠ {String(error)}</div>}
      <div className="metrics-grid">
        {metrics.map(({ label, value, badge, Icon }) => (
          <div key={label} className="metric-card">
            <div className="metric-header"><span className="metric-label">{label}</span><Icon /></div>
            <div className="metric-value">{value}</div>
            <div className="metric-badge">{badge}</div>
          </div>
        ))}
      </div>
      <div className="table-header"><span className="table-title">Recent Contacts</span></div>
      <div className="contact-table">
        ...
        {contactLoading && <div className="table-row" style={{ opacity:.5 }}>Loading…</div>}
        {!contactLoading && tableRows.length === 0 && <div className="table-row" style={{ opacity:.5 }}>No contacts yet.</div>}
        {tableRows.map((row, i) => (
...
// ── Forbidden ──────────────────────────────────────────────
const Forbidden = () => (
  <div style={{ padding:"4rem 2rem", textAlign:"center", opacity:.55 }}>
    <div style={{ fontSize:"2.5rem", marginBottom:"1rem" }}>🔒</div>
    <h2 style={{ fontFamily:"Rajdhani, sans-serif", fontSize:"1.5rem", marginBottom:".5rem" }}>Access Restricted</h2>
    <p style={{ fontFamily:"Space Mono, monospace", fontSize:".8rem" }}>This page is only accessible to super admins.</p>
  </div>
);
...
const AdminShell = ({ theme }) => {
  ...
  const visibleNav = BASE_NAV.filter((item) => !item.superadminOnly || isSuperAdmin);

  const renderPage = () => {
    switch (page) {
      case "blog":      return <AdminBlog />;
      case "projects":  return <AdminProjects />;
      case "dashboard": return isSuperAdmin ? <Dashboard /> : <Forbidden />;
      default:          return isSuperAdmin ? <Dashboard /> : <AdminProjects />;
    }
  };
    ...
    <div data-theme={theme}>
      <div className="admin-shell">
        <aside className="sidebar">
          <Link to="/"><div className="sidebar-brand"><span className="brand-name">Software Admin</span></div></Link>
          <nav>
          ...
          <div className="sidebar-user">
            ...
            <div className="user-role">{ROLE_LABELS[admin?.role] ?? admin?.role}</div>
            <div style={{ marginTop:"10px", fontSize:".75rem", cursor:"pointer", opacity:.5, fontFamily:"Space Mono,monospace" }} onClick={logout}>
              sign out →
            </div>
          </div>
        </aside>
...
const AdminRoot = ({ theme }) => {
  ...
  useEffect(() => { setAuthenticated(!!admin); }, [admin]);
