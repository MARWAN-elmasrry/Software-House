...
const initialProjects = [
  { id: 1, name: "Website Redesign", status: "active", client: "Acme Corp", due: "Apr 15, 2026", progress: 72 },
  { id: 2, name: "Mobile App v2",    status: "review", client: "Beta LLC",  due: "May 01, 2026", progress: 90 },
  { id: 3, name: "API Integration",  status: "active", client: "Gamma Inc", due: "Mar 30, 2026", progress: 40 },
  { id: 4, name: "Dashboard UI",     status: "done",   client: "Delta Co",  due: "Mar 01, 2026", progress: 100 },
];
...
export const AdminProjects = () => {
  ...
  const openEdit = (p) => { setForm({ name: p.name, client: p.client, due: p.due, status: p.status, progress: p.progress }); setEditId(p.id); setModal("edit"); };
  const close    = () => { setModal(null); setEditId(null); };

  const handleSave = () => {
    if (!form.name.trim()) return;
    if (modal === "add") setProjects(prev => [...prev, { ...form, id: Date.now() }]);
    else setProjects(prev => prev.map(p => p.id === editId ? { ...p, ...form } : p));
    close();
  };

  const handleDelete = (id) => { setProjects(prev => prev.filter(p => p.id !== id)); setDeleteId(null); };
    ...
    <div className="ab">
      ...
      <div className="ab__topbar">
        <div>
          ...
          <p className="ab__subtitle">{projects.length} total · {projects.filter(p => p.status === "active").length} active</p>
        </div>
      ...
      <div className="ap-table">
        ...
        {projects.map(p => (
          <div className="ap-table__row" key={p.id}>
            ...
            <div className="ap-table__actions">
              ...
              <button className="ab-icon-btn ab-icon-btn--danger" onClick={() => setDeleteId(p.id)} title="Delete">
      ...
      {modal && <Modal form={form} onChange={handleField} onSave={handleSave} onClose={close} isEdit={modal === "edit"} />}
